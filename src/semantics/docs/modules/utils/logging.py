"""Logging utilities for docs modules.

Re-exports shared UI functions from the platform-level ``ui`` module and
provides docs-specific helpers (gray debug output, external-logging config).
"""
...
# ---------------------------------------------------------------------------
# Docs-specific: gray debug output
# ---------------------------------------------------------------------------
...
def configure_external_logging(debug: bool) -> None:
    """Quieten chatty third-party loggers unless debug mode is on."""

    # Override ORT_LOGGING_LEVEL for debug mode (wrapper sets FATAL by default)
    if debug:
        os.environ["ORT_LOGGING_LEVEL"] = "INFO"

    # Import ORT with fd-level stderr suppression to catch the C++
    # device_discovery warning that fires at shared-library load time.
    # Must use fd-level redirect even in debug mode since C++ writes
    # directly to fd 2, bypassing Python's sys.stderr.
