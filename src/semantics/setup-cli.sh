...
echo "--- Processing module: $module_name found at $module_dir using pip/venv ---"
...
echo "Creating venv at $venv_path (with --system-site-packages to reuse shared CUDA libs)"
python3 -m venv --system-site-packages "$venv_path"

# --- Install Requirements ---
echo "Upgrading pip in $venv_path"
...
"$venv_python" -m pip install --no-cache-dir -r "$install_source"
...
if [ -f "$override_file" ]; then
    ...
    # Read the override file line-by-line and install each non-empty, non-comment spec
    echo "Pre-downloading wheels into $wheelhouse"
    python3 -m pip download --progress-bar off --dest "$wheelhouse" -r "$install_source" || true
    while IFS= read -r line || [ -n "$line" ]; do
    "$venv_python" -m pip install --no-cache-dir --find-links "$wheelhouse" -r "$install_source"
...
if [ ! -f "$lock_file" ]; then
    ...
    "$venv_python" -m pip freeze --exclude-editable > "$lock_file"
fi
