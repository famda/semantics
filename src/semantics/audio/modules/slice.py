...
Extracts a time range from an input audio or video file using FFmpeg
stream-copy for speed. Falls back to re-encoding when stream-copy fails
and ``fallback_reencode`` is enabled in the config.
...
def handle(
    ...
    *,
    ...
    """Slice a media file to the requested time range.
...
def _build_ffmpeg_cmd(
    ...
    *,
    ...
    if codec == "copy":
        ...
        # Re-encode audio to PCM WAV (safe for any downstream processing)
        cmd += ["-c:a", "pcm_s16le", "-ac", "1", "-ar", "16000"]
        # Drop video stream when re-encoding for audio pipeline
        cmd += ["-vn"]
    else:
        cmd += ["-c", codec]
